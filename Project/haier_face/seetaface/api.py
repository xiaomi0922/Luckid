...
FACE_AGE = 0x00000200
#口罩人脸识别
MASK_FACERECOGNITION = 0x00000800
...
class SeetaFace_Mask(object):
    ...
    def _dll_func_def(self):
        ...
        self._MaskExtract = dll.MaskExtract
        self._MaskExtract.restype = c_int32
        self._MaskExtract.argtypes = (POINTER(SeetaImageData), POINTER(SeetaPointF), POINTER(c_float))

        self._MaskCalculateSimilarity = dll.MaskCalculateSimilarity
        self._MaskCalculateSimilarity.restype = c_float
        self._MaskCalculateSimilarity.argtypes = (POINTER(c_float), POINTER(c_float))
    ...
    def set(self, yaw_low_threshold: int,
        ...
        :param roll_high_threshold:
        """
        self._set(yaw_low_threshold, yaw_high_threshold, pitch_low_threshold, pitch_high_threshold, roll_low_threshold,
                  roll_high_threshold)
    ...
    def MaskExtract(self, frame: np.array, points: List[SeetaPointF]) -> float:
        """
        在一张图片中提取戴口罩人脸关键点区域的人脸的特征值
        :param frame:
        :param points:
        :return:
        """
        self.check_init(MASK_FACERECOGNITION)
        seetaImageData = get_seetaImageData_by_numpy(frame)
        feature = (c_float * 512)()
        self._MaskExtract(seetaImageData, points, feature)
        return feature

    def MaskCalculateSimilarity(self, features1, features2) -> float:
        self.check_init(MASK_FACERECOGNITION)
        return self._MaskCalculateSimilarity(features1, features2)